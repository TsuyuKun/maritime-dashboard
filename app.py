...
ships = [
    {
        ...
        "timeline": [{"time": "10:30", "km": "KM 30", "cond": "🌧️ Hujan"}, {"time": "10:45", "km": "KM 45", "cond": "☁️ Tebal"}, {"time": "11:00", "km": "KM 60", "cond": "⛅ Berawan"}, {"time": "11:15", "km": "Arrival", "cond": "☀️ Cerah"}]
    },
    {
        ...
        "timeline": [{"time": "Now", "km": "ALKI I", "cond": "☀️ Cerah"}, {"time": "+15m", "km": "KM 10", "cond": "☀️ Cerah"}, {"time": "+30m", "km": "KM 20", "cond": "⛅ Berawan"}]
    }
]
...
if img_buf:
    ...
    for s in ships:
        ...
        # POPUP KEMBALI LENGKAP
        t_rows = "".join([f"<li><b>{i['time']}</b>: {i['cond']}</li>" for i in s['timeline']])
        p_html = f"""<div style="font-family: Arial; width: 200px; font-size: 11px;">
            <b style="color: #2980b9; font-size: 13px;">{s['name']}</b><br>{s['type']}<hr style="margin:4px 0;">
            <b>SPD:</b> {s['speed']} | <b>HDG:</b> {s['course']}°<br><b>ETA:</b> {s['eta']}<hr style="margin:4px 0;">
            <b>Forecast:</b><ul style="padding-left:15px; margin:5px 0; list-style:none;">{t_rows}</ul></div>"""
        
        folium.Marker(location=[s['lat'], s['lon']], popup=folium.Popup(p_html, max_width=250),
                      icon=folium.DivIcon(html=f'<div style="transform: rotate({s["course"]}deg); color:#00f2ff; font-size:24px;">➤</div>')).add_to(m)
